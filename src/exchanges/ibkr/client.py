...
from .util import Event, UNSET_DOUBLE, UNSET_INTEGER, dataclassAsTuple, getLoop, run
...
class Client:
    ...
    * Optional ``wrapper.tcpDataProcessed()`` method;
    ...
    def __init__(self, wrapper):
        ...
        self.conn.add_data_callback(self._onSocketHasData)
        self.conn.add_disconnected_callback(self._onSocketDisconnected)
    ...
    async def connectAsync(self, host, port, clientId, timeout=2.0):
        try:
            ...
            self.apiStart.connect(lambda: future.set_result(None))
