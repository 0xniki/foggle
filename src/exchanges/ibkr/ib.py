...
import logging
import time
from typing import Any, Awaitable, Dict, Iterator, List, Optional, Union, Callable
...
class IB:
    ...
    **user code may not block for too long**.
    ...
    def __init__(self):
        ...
        self.errorEvent.connect(self._onError)
        self.client.apiEnd.connect(self.disconnectedEvent.emit)
        self._logger = logging.getLogger("ib_async.ib")

    def _createEvents(self):
        self.connectedEvent = util.Event("connectedEvent")
        self.disconnectedEvent = util.Event("disconnectedEvent")
        self.updateEvent = util.Event("updateEvent")
        self.pendingTickersEvent = util.Event("pendingTickersEvent")
        self.barUpdateEvent = util.Event("barUpdateEvent")
        self.newOrderEvent = util.Event("newOrderEvent")
        self.orderModifyEvent = util.Event("orderModifyEvent")
        self.cancelOrderEvent = util.Event("cancelOrderEvent")
        self.openOrderEvent = util.Event("openOrderEvent")
        self.orderStatusEvent = util.Event("orderStatusEvent")
        self.execDetailsEvent = util.Event("execDetailsEvent")
        self.commissionReportEvent = util.Event("commissionReportEvent")
        self.updatePortfolioEvent = util.Event("updatePortfolioEvent")
        self.positionEvent = util.Event("positionEvent")
        self.accountValueEvent = util.Event("accountValueEvent")
        self.accountSummaryEvent = util.Event("accountSummaryEvent")
        self.pnlEvent = util.Event("pnlEvent")
        self.pnlSingleEvent = util.Event("pnlSingleEvent")
        self.scannerDataEvent = util.Event("scannerDataEvent")
        self.tickNewsEvent = util.Event("tickNewsEvent")
        self.newsBulletinEvent = util.Event("newsBulletinEvent")
        self.wshMetaEvent = util.Event("wshMetaEvent")
        self.wshEvent = util.Event("wshEvent")
        self.errorEvent = util.Event("errorEvent")
        self.timeoutEvent = util.Event("timeoutEvent")
    ...
    async def connectAsync(
        ...
        try:
            ...
            await self.client.connectAsync(config["host"], config["port"], clientId, timeout)
